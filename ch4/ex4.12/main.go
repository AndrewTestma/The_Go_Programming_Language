...
func fetcher(comicNums chan int, comics chan Comic, done chan int) {
	for n := range comicNums {
		...
		if err != nil {
			logrus.Printf("can't get comic %d: %s", n, err)
			continue
		}
		comics <- comic
	}
...
func main() {
	...
	case "get":
		...
		comic, err := getComic(n)
		if err != nil {
			logrus.Fatal("Error getting comic", err)
		}
		fmt.Println(comic)
	case "index":
		if len(os.Args) != 3 {
		...
		err := index(os.Args[2])
		if err != nil {
			logrus.Fatal("Error serializing indexes .", err)
		}
	case "search":
		if len(os.Args) != 4 {
			usageDie()
		...
		if err != nil {
			logrus.Fatal("Error searching index", err)
		}
	default:
		usageDie()
	}
